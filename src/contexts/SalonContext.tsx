...
export interface SalonContextType {
    ...
    updatePackageSession: (id: number) => void;
...
export function SalonProvider({ children }: { children: ReactNode }) {
    ...
    const updateClientMutation = useMutation({
        mutationFn: async ({ id, updates }: { id: number; updates: Partial<Client> }) => {
            ...
            const { error } = await supabase.from('clients').update(allowedUpdates).eq('id', id);
            if (error) throw new Error(error.message);
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['clients'] });
        ...
        mutationFn: async (payload: {
            ...
            const { data: saleData, error: saleError } = await supabase
                ...
                .insert([{
                    ...
                    payment_status: payload.status === 'pago' ? 'paid' : 'pending',
                    notes: payload.note || '',
